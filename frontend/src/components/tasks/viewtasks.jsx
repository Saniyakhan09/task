...
const Viewtasks = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const res = await fetch(`https://taskmangementproject.onrender.com/task/tasks/${adminId}`, {
          method: "GET",
          headers: {
  ...
  const handledelete = async (taskId) => {
    try {
      const res = await fetch(`https://taskmangementproject.onrender.com/task/delete/${taskId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
